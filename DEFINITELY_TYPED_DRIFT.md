...
- Data plan version objects are typed locally instead of requiring consumers to
  install `@mparticle/data-planning-models` just to consume Web SDK types.
...
- Added public Rokt type aliases used by consumers:
  `RoktSelection`, `RoktPlacement`, `RoktPlacementEvent`, and `RoktAttributes`.
