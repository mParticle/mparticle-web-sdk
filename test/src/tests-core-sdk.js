...
describe('core SDK', function() {
    ...
    it('creates a new dateLastEventSent when logging an event, and retains the previous one when ending session', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            Should(testEvent1.data.timestamp_unixtime_ms).not.equal(
                testEvent2.data.timestamp_unixtime_ms,
            );
            Should(testEvent2.data.timestamp_unixtime_ms).equal(
                sessionEndEvent.data.timestamp_unixtime_ms,
            );
    ...
    it('should set app version on the payload', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            testEventBatch.application_info.should.have.property(
                'application_version',
                '1.0',
            );
    ...
    it('should sanitize event attributes', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            const sanitizedEvent = findEventFromRequest(
                fetchMock.calls(),
                'sanitized event',
            );

            sanitizedEvent.data.custom_attributes.should.have.property(
                'key1',
                'value1',
            );
            sanitizedEvent.data.custom_attributes.should.have.property(
            ...
            sanitizedEvent.data.custom_attributes.should.not.have.property(
                'ishouldberemoved',
            );
            sanitizedEvent.data.custom_attributes.should.not.have.property(
                'ishouldalsoberemoved',
            );
            sanitizedEvent.data.custom_attributes.should.not.have.property(
                'removeme',
            );
    ...
    it('sanitizes attributes when attrs are provided', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            checkoutEvent.data.custom_attributes.should.not.have.property(
                'invalid',
            );
            ...
            addToCartEvent.data.custom_attributes.should.not.have.property(
                'invalid',
            );
            ...
            purchaseEvent.data.custom_attributes.should.not.have.property(
                'invalid',
            );
            ...
            const promotionViewEvent = findEventFromRequest(
                fetchMock.calls(),
                'view',
            );
            promotionViewEvent.data.custom_attributes.should.not.have.property(
                'invalid',
            );
            promotionViewEvent.data.custom_attributes.should.have.property(
                'valid',
            );
            ...
            refundEvent.data.custom_attributes.should.not.have.property(
                'invalid',
            );
    ...
    it('creates a new session when elapsed time between actions is greater than session timeout', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            waitForCondition(() => {
                ...
                testEvent.data.session_uuid.should.not.equal(
                    testEvent2.data.session_uuid,
                );
    ...
    it('should end session when last event sent is outside of sessionTimeout', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            waitForCondition(() => {
                ...
                testEvent2.data.session_uuid.should.equal(
                    testEvent.data.session_uuid,
                );
                testEvent3.data.session_uuid.should.not.equal(
                    testEvent.data.session_uuid,
                );
                clock.restore();
                done();
    ...
    it('should not end session when end session is called within sessionTimeout timeframe', function(done) {
        ...
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            waitForCondition(() => {
                ...
                let sessionEndEvent = findEventFromRequest(
                    fetchMock.calls(),
                    'session_end',
                );
                ...
                const sid = mParticle
                    .getInstance()
                    ._Persistence.getLocalStorage().gs.sid;
                ...
                clock.tick(70000);
                sessionEndEvent = findEventFromRequest(
                    fetchMock.calls(),
                    'session_end',
                );
                ...
                testEvent3.data.session_uuid.should.equal(
                    testEvent2.data.session_uuid,
                );
    ...
    it('should set the sessionId from memory on the payload', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            mParticle.logEvent('Test Event');
            const testEvent = findEventFromRequest(
                fetchMock.calls(),
    ...
    it('should update session start date when manually ending session then starting a new one', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            const testEventSessionStartTime =
            ...
            const sessionEndEventSessionStartDate =
                sessionEndEvent.data.session_start_unixtime_ms;
            sessionEndEventSessionStartDate.should.equal(
                testEventSessionStartTime,
            );
            ...
            const testEvent2SessionStartDate =
                testEvent2.data.session_start_unixtime_ms;
            testEvent2SessionStartDate.should.be.above(
                sessionEndEventSessionStartDate,
            );
    ...
    it('should update session start date when session times out, then start a new one', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            waitForCondition(() => {
                ...
                const testEventSessionStartDate =
                    testEvent.data.session_start_unixtime_ms;
    ...
    it('should load SDK with the included api on init and not send events to previous apikey in persistence', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            const testEvent1URL = findRequestURL(
                fetchMock.calls(),
                'Test Event1',
            );
            ...
            waitForCondition(() => {
                ...
                const testEvent2URL = findRequestURL(
                    fetchMock.calls(),
                    'Test Event2',
                );
    ...
    it('should use default urls if no custom urls are set in config object', function(done) {
        ...
        mParticle
            .getInstance()
            ._Store.SDKConfig.v1SecureServiceUrl.should.equal(
                Constants.DefaultBaseUrls.v1SecureServiceUrl,
            );
        mParticle
            .getInstance()
            ._Store.SDKConfig.v2SecureServiceUrl.should.equal(
                Constants.DefaultBaseUrls.v2SecureServiceUrl,
            );
        mParticle
            .getInstance()
            ._Store.SDKConfig.v3SecureServiceUrl.should.equal(
                Constants.DefaultBaseUrls.v3SecureServiceUrl,
            );
        mParticle
            .getInstance()
            ._Store.SDKConfig.configUrl.should.equal(
                Constants.DefaultBaseUrls.configUrl,
            );
        mParticle
            .getInstance()
            ._Store.SDKConfig.identityUrl.should.equal(
                Constants.DefaultBaseUrls.identityUrl,
            );
        mParticle
            .getInstance()
            ._Store.SDKConfig.aliasUrl.should.equal(
                Constants.DefaultBaseUrls.aliasUrl,
            );
    ...
    it('should have default urls if no custom urls are set in config object, but use custom urls when they are set', function(done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            waitForCondition(() => {
                ...
                const testEventURL = findRequestURL(
                    fetchMock.calls(),
                    'Test Event',
                );
                ...
                mParticle.Identity.login({
                    userIdentities: { customerid: 'test1' },
                });
                ...
                waitForCondition(() => {
                    ...
                    fetchMock.post(
                        'https://custom-alias/test_key/Alias',
                        HTTP_ACCEPTED,
                    );
    ...
    it('should add onCreateBatch to _Store.SDKConfig if onCreateBatch is provide on mParticle.config object', function(done) {
        ...
        mParticle.init(apiKey, mParticle.config);
        (typeof mParticle.getInstance()._Store.SDKConfig
            .onCreateBatch).should.equal('function');
    ...
    it('should not add onCreateBatch to _Store.SDKConfig if it is not a function', function(done) {
        ...
        (typeof mParticle.getInstance()._Store.SDKConfig
            .onCreateBatch).should.equal('undefined');
    ...
    it('should fetch from /config and keep everything properly on the store', function(done) {
        ...
        waitForCondition(() => {
            return mParticle.getInstance()._Store.configurationLoaded === true;
        }).then(() => {
            ...
            mParticle.getInstance()._Store.SDKConfig.minWebviewBridgeVersion =
                config.minWebviewBridgeVersion;
            mParticle.getInstance()._Store.SDKConfig.workspaceToken =
    ...
    it('should not set the wrapper sdk info in Store when mParticle._setWrapperSDKInfo() method is called if init not called', function(done) {
        ...
        (
            mParticle.getInstance()._Store.wrapperSDKInfo.version === null
        ).should.equal(true);
        mParticle
            .getInstance()
            ._Store.wrapperSDKInfo.isInfoSet.should.equal(false);
    ...
    it('should set the wrapper sdk info in Store when mParticle._setWrapperSDKInfo() method is called after init is called', async function() {
        ...
        mParticle
            .getInstance()
            ._Store.wrapperSDKInfo.name.should.equal('flutter');
        mParticle
            .getInstance()
            ._Store.wrapperSDKInfo.version.should.equal('1.0.3');
        mParticle
            .getInstance()
            ._Store.wrapperSDKInfo.isInfoSet.should.equal(true);
    });
    ...
    describe('pod feature flag', function() {
        ...
        it('should use US1 endpoints for apiKeys that do not start with a prefix', function(done) {
            ...
            mParticle
                .getInstance()
                ._Store.SDKConfig.identityUrl.should.equal(
                    URLs[silo].identityUrl,
                );
            mParticle
                .getInstance()
                ._Store.SDKConfig.v1SecureServiceUrl.should.equal(
                    URLs[silo].v1SecureServiceUrl,
                );
            mParticle
                .getInstance()
                ._Store.SDKConfig.v2SecureServiceUrl.should.equal(
                    URLs[silo].v2SecureServiceUrl,
                );
            mParticle
                .getInstance()
                ._Store.SDKConfig.v3SecureServiceUrl.should.equal(
                    URLs[silo].v3SecureServiceUrl,
                );
        ...
        it('should prioritize configured URLs over direct URL mapping', function(done) {
            ...
            mParticle
                .getInstance()
                ._Store.SDKConfig.aliasUrl.should.equal(aliasUrl);
            mParticle
                .getInstance()
                ._Store.SDKConfig.configUrl.should.equal(configUrl);
            mParticle
                .getInstance()
                ._Store.SDKConfig.identityUrl.should.equal(identityUrl);
            mParticle
                .getInstance()
                ._Store.SDKConfig.v1SecureServiceUrl.should.equal(
                    URLs[silo].v1SecureServiceUrl,
