...
describe('batch uploader', () => {
    ...
    describe('AST background events fired during page events', () => {
        describe('should not fire AST background events if flag is astBackgroundEvents "False"', () => {
            ...
            it('should not have an AST of background event when visibility changes', async () => {
                ...
                expect(
                    beaconSpy.calledOnce,
                    'Expected beacon to be called once',
                ).to.be.true;
                ...
                expect(batch.events, 'Expected beacon data to have events').to
                    .exist;
                expect(
                    batch.events.length,
                    'Expected beacon data to have at least one event',
                ).to.equal(3);
                ...
                expect(event2.event_type).to.equal(
                    'application_state_transition',
                );
                expect(event2.event_type).to.equal(
                    'application_state_transition',
                );
                expect(event2.data.application_transition_type).to.equal(
        ...
        describe('should fire AST background events if flag is astBackgroundEvents "True"', () => {
            ...
            it('should add application state transition event when visibility changes to hidden', async () => {
                ...
                expect(lastEvent.event_type).to.equal(
                    'application_state_transition',
                );
                expect(lastEvent.data.application_transition_type).to.equal(
            ...
            it('should add application state transition event before pagehide', async () => {
                ...
                try {
                    ...
                    // Get the beacon call data
                    const beaconCall = beaconSpy.getCall(0);
                    expect(beaconCall, 'Expected beacon call to exist').to
                        .exist;
                    ...
                    expect(events[events.length - 1].event_type).to.equal(
                        'application_state_transition',
                    );
                    expect(events[events.length - 2].data.event_name).to.equal(
                        'Test Event',
                    );
                } finally {
            ...
            it('should create application state transition event with correct properties on event and batch', async () => {
                ...
                const consentState =
                    window.mParticle.Consent.createConsentState();
                ...
                expect(batch.consent_state.ccpa).to.have.property(
                    'data_sale_opt_out',
                );
                expect(
                    batch.consent_state.ccpa.data_sale_opt_out,
                ).to.have.property('consented');
                expect(
                    batch.consent_state.ccpa.data_sale_opt_out.consented,
                ).to.equal(true);
                expect(
                    batch.consent_state.ccpa.data_sale_opt_out,
                ).to.have.property('timestamp_unixtime_ms');
                expect(
                    batch.consent_state.ccpa.data_sale_opt_out,
                ).to.have.property('document');
                expect(batch.consent_state.gdpr).to.have.property(
                    'test purpose',
                );
                expect(
                    batch.consent_state.gdpr['test purpose'],
                ).to.have.property('consented');
                expect(
                    batch.consent_state.gdpr['test purpose'].consented,
                ).to.equal(false);
                expect(
                    batch.consent_state.gdpr['test purpose'],
                ).to.have.property('timestamp_unixtime_ms');
                ...
                expect(astEvent.event_type).to.equal(
                    'application_state_transition',
                );
                expect(astEvent.data.active_time_on_site_ms).to.be.a('number');
                expect(astEvent.data.application_transition_type).to.equal(
                    'application_background',
                );
                ...
                expect(astEvent.data.session_start_unixtime_ms).to.be.a(
                    'number',
                );
    ...
    describe('AST Debouncing events', () => {
        ...
        it('should only fire a single AST when another visibility event happens within the debounce time window', async () => {
            ...
            expect(beaconSpy.calledOnce, 'Expected beacon to be called once').to
                .be.true;
            ...
            expect(beaconSpy.calledOnce, 'Expected beacon to be called twice')
                .to.be.true;
        });
        ...
        it('should fire multiple ASTs when another visibility event happens outside the debounce time window', async () => {
            ...
            expect(beaconSpy.calledTwice, 'Expected beacon to be called twice')
                .to.be.true;
        });
    });
        ...
        describe('#queueEvent', () => {
            ...
            it('should add events in the order they are received', () => {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    expect(uploader.eventsQueuedForProcessing[0]).to.eql(
                        event1,
                    );
                    expect(uploader.eventsQueuedForProcessing[1]).to.eql(
                        event2,
                    );
                    expect(uploader.eventsQueuedForProcessing[2]).to.eql(
                        event3,
                    );
                });
            });
        ...
        describe('#uploadBatches', () => {
            ...
            it('should not throw an error when upload is called while storage has not been created yet', async () => {
                ...
                expect(() => {
                    ...
                    TypeError,
                    /Cannot read properties of null \(reading 'prepareAndUpload'\)/,
                );
            });
            ...
            waitForCondition(hasIdentifyReturned)
                .then(() => {
                    ...
                    const batchQueue: Batch[] =
                        uploader.batchesQueuedForProcessing;
    ...
    describe('Offline Storage Enabled', () => {
        ...
        it('should store events in Session Storage in order of creation', (done) => {
            ...
            waitForCondition(hasIdentifyReturned)
                .then(() => {
                    ...
                    expect(
                        storedEvents[0],
                        'Local Storage: Session Start',
                    ).to.eql(eventQueue[0]);
                    expect(storedEvents[1], 'Local Storage: AST').to.eql(
                        eventQueue[1],
                    );
                    expect(
                        storedEvents[2],
                        'Local Storage: Test Event 0',
                    ).to.eql(eventQueue[2]);
        ...
        it('should purge events from Session Storage upon Batch Creation', async () => {
            ...
            waitForCondition(hasIdentifyReturned)
                .then(async () => {
                    ...
                    expect(uploader.eventsQueuedForProcessing.length).to.equal(
                        3,
                    );
                    expect(uploader.batchesQueuedForProcessing.length).to.equal(
                        0,
                    );
                    ...
                    // Manually initiate the upload process - turn event into batches and upload the batch
                    await window.mParticle
                        .getInstance()
                        ._APIClient.uploader.prepareAndUpload();
        ...
        it('should save batches in sequence to Local Storage when an HTTP 500 error is encountered', async () => {
            ...
            waitForCondition(hasIdentifyReturned)
                .then(async () => {
                    ...
                    expect(window.localStorage.getItem(batchStorageKey)).to.be
                        .ok;
        ...
        it('should save batches in sequence to Local Storage when upload is interrupted', async () => {
            ...
            waitForCondition(hasIdentifyReturned)
                ...
                .catch((e) => {
                    console.log(
                        'should save batches in sequence to Local Storage when upload is interrupted',
                    );
                });
        });
