...
describe('feature-flags', function () {
    ...
    describe('capture integration specific ids', () => {
        ...
        it('should capture click ids when feature flag is true', async () => {
            ...
            const clickIdSpy = sinon.spy(
                window.mParticle.getInstance()._IntegrationCapture,
                'getClickIdsAsCustomFlags',
            );
            ...
            expect(
                window.mParticle.getInstance()._IntegrationCapture.clickIds,
            ).to.deep.equal({
                fbclid: `fb.1.${initialTimestamp}.1234`,
                _fbp: '54321',
            });
            expect(captureSpy.called, 'capture()').to.equal(true);
            expect(clickIdSpy.called, 'getClickIdsAsCustomFlags').to.equal(
                true,
            );
        });
        ...
        it('should NOT capture click ids when feature flag is false', async () => {
            ...
            expect(window.mParticle.getInstance()._IntegrationCapture.clickIds)
                .not.be.ok;
