...
describe('kit blocking', () => {
    const kitBlockerDataPlan: KitBlockerDataPlan = {
        document: dataPlan,
    ...
    describe('kitBlocker', () => {
        it('kitBlocker should parse data plan into dataPlanMatchLookups properly', function (done) {
            ...
            const dataPlanMatchLookups = kitBlocker.dataPlanMatchLookups;
            dataPlanMatchLookups.should.have.property(
                'custom_event:search:Search Event',
                true,
            );
            dataPlanMatchLookups.should.have.property(
                'screen_view::another new screenview event',
                {},
            );
            dataPlanMatchLookups.should.have.property(
                'custom_event:location:locationEvent',
                { foo: true, 'foo foo': true, 'foo number': true },
            );
            dataPlanMatchLookups.should.have.property(
            ...
            dataPlanMatchLookups.should.have.property(
                'product_action:add_to_cart:ProductAttributes',
                true,
            );
            ...
            dataPlanMatchLookups.should.have.property(
                'promotion_action:view:ProductAttributes',
                true,
            );
            dataPlanMatchLookups.should.have.property(
                'custom_event:navigation:TestEvent',
                true,
            );
            dataPlanMatchLookups.should.have.property(
                'product_impression:ProductAttributes',
                {
                    allowedAttr1: true,
                    allowedAttr2: true,
                },
            ...
            dataPlanMatchLookups.should.have.property(
                'screen_view::A New ScreenViewEvent',
                true,
            );
            dataPlanMatchLookups.should.have.property(
                'screen_view::my screeeen',
                {
                    test2key: true,
                    test1key: true,
                },
            ...
            dataPlanMatchLookups.should.have.property(
                'custom_event:navigation:something something something',
                true,
            );
            ...
            dataPlanMatchLookups.should.have.property(
                'custom_event:social:SocialEvent',
                {},
            );
            dataPlanMatchLookups.should.have.property(
            ...
            dataPlanMatchLookups.should.have.property(
                'product_action:purchase:ProductAttributes',
                {
                    plannedAttr1: true,
                    plannedAttr2: true,
                },
            ...
            dataPlanMatchLookups.should.have.property(
                'promotion_action:click:ProductAttributes',
                true,
            );
    ...
    describe('kitblocking - events and event attributes', () => {
        ...
        it('should transform an unplanned event to null if blok.ev = true', function (done) {
            ...
            const transformedEvent =
                kitBlocker.transformEventAndEventAttributes(event);
        ...
        it('should transform EventAttributes if an event attribute is not planned and blok.ea = true', function (done) {
            ...
            const transformedEvent =
                kitBlocker.transformEventAndEventAttributes(event);
            transformedEvent.EventAttributes.should.not.have.property(
                'unplannedAttr',
            );
        ...
        it('should include unplanned event attributes if additionalProperties = true and blok.ea = true', function (done) {
            ...
            const transformedEvent =
                kitBlocker.transformEventAndEventAttributes(event);
            transformedEvent.EventAttributes.should.have.property('foo', 'hi');
            transformedEvent.EventAttributes.should.have.property(
                'keyword2',
                'test',
            );
        ...
        it('should block any unplanned event attributes if custom attributes is empty, additionalProperties = false, and block.ea = true.', function (done) {
            ...
            const transformedEvent =
                kitBlocker.transformEventAndEventAttributes(event);
            Object.keys(transformedEvent.EventAttributes).length.should.equal(
                0,
            );
        ...
        it('should not block any unplanned event attributes if custom attributes is empty, additionalProperties = true, and block.ea = true.', function (done) {
            ...
            const socialDataPoint =
                dataPlan.dtpn.vers.version_document.data_points.find(
                    (dataPoint) => {
                        return (
                            dataPoint?.match?.criteria?.event_name ===
                            'SocialEvent'
                        );
                    },
                );
            socialDataPoint.validator.definition.properties.data.properties.custom_attributes.additionalProperties =
                true;
    ...
    describe('kit blocking - user attributes/identities', () => {
        ...
        it('should block any unplanned user attributes when blok.ua = true and additionalPropertes = false', function (done) {
            ...
            const transformedEvent = kitBlocker.transformUserAttributes(event);
            transformedEvent.UserAttributes.should.have.property(
                'my attribute',
                'test1',
            );
            transformedEvent.UserAttributes.should.have.property(
            ...
            transformedEvent.UserAttributes.should.not.have.property(
                'unplanned attribute',
            );
        ...
        it('isAttributeKeyBlocked should return false for attributes that are blocked and true for properties that are not', function (done) {
            ...
            userAttributeDataPoint.validator.definition.additionalProperties =
                true;

            const kitBlocker2 = new KitBlocker(
                kitBlockerDataPlan,
                window.mParticle.getInstance(),
            );
        ...
        it('should not block any unplanned user identities when blok.id = true and additionalProperties = true', function (done) {
            ...
            const transformedEvent = kitBlocker.transformUserIdentities(event);

            transformedEvent.UserIdentities.filter(
                (UI) => UI.Type === 1,
            )[0].should.have.property('Identity', 'customerid1');
            transformedEvent.UserIdentities.filter(
                (UI) => UI.Type === 7,
            )[0].should.have.property('Identity', 'email@gmail.com');
            transformedEvent.UserIdentities.filter(
                (UI) => UI.Type === 4,
            )[0].should.have.property('Identity', 'GoogleId');

            done();
        });

        it('should block user identities when additional properties = false and blok.id = true', function (done) {
            const userIdentityDataPoint =
                dataPlan.dtpn.vers.version_document.data_points.find(
                    (dataPoint) => {
            ...
            userIdentityDataPoint.validator.definition.additionalProperties =
                false;
            ...
            transformedEvent.UserIdentities.find(
                (UI) => UI.Type === 1,
            ).should.have.property('Identity', 'customerid1');
            transformedEvent.UserIdentities.find(
                (UI) => UI.Type === 7,
            ).should.have.property('Identity', 'email@gmail.com');
            (
                transformedEvent.UserIdentities.find((UI) => UI.Type === 4) ===
                undefined
            ).should.equal(true);
    ...
    describe('kit blocking - product attributes', () => {
        ...
        it('should transform productAttributes if an product attribute is not planned, additionalProperties = false, and blok.ea = true', function (done) {
            ...
            const transformedEvent =
                kitBlocker.transformProductAttributes(event);
            transformedEvent.ProductAction.ProductList[0].Attributes.should.not.have.property(
                'unplannedAttr1',
            );
            transformedEvent.ProductAction.ProductList[0].Attributes.should.have.property(
                'plannedAttr1',
            );
            transformedEvent.ProductAction.ProductList[0].Attributes.should.have.property(
                'plannedAttr2',
            ...
            transformedEvent.ProductAction.ProductList[1].Attributes.should.not.have.property(
                'unplannedAttr1',
            );
            transformedEvent.ProductAction.ProductList[1].Attributes.should.have.property(
                'plannedAttr1',
            );
            transformedEvent.ProductAction.ProductList[1].Attributes.should.have.property(
                'plannedAttr2',
        ...
        it('should transform productAttributes in product impressions if an product attribute is not planned, additionalProperties = false,  and blok.ea = true', function (done) {
            ...
            transformedEvent.ProductImpressions[0].ProductList[0].Attributes.should.have.property(
                'allowedAttr1',
                'val4',
            );
            transformedEvent.ProductImpressions[0].ProductList[0].Attributes.should.have.property(
            ...
            transformedEvent.ProductImpressions[0].ProductList[0].Attributes.should.not.have.property(
                'unplannedAttr1',
            );
            transformedEvent.ProductImpressions[0].ProductList[0].Attributes.should.not.have.property(
                'unplannedAttr2',
            );
            transformedEvent.ProductImpressions[0].ProductList[1].Attributes.should.have.property(
                'allowedAttr1',
                'val4',
            );
            transformedEvent.ProductImpressions[0].ProductList[1].Attributes.should.have.property(
            ...
            transformedEvent.ProductImpressions[0].ProductList[1].Attributes.should.not.have.property(
                'unplannedAttr1',
            );
            transformedEvent.ProductImpressions[0].ProductList[1].Attributes.should.not.have.property(
                'unplannedAttr2',
            ...
            transformedEvent.ProductImpressions[1].ProductList[0].Attributes.should.have.property(
                'allowedAttr1',
                'val4',
            );
            transformedEvent.ProductImpressions[1].ProductList[0].Attributes.should.have.property(
            ...
            transformedEvent.ProductImpressions[1].ProductList[0].Attributes.should.not.have.property(
                'unplannedAttr1',
            );
            transformedEvent.ProductImpressions[1].ProductList[0].Attributes.should.not.have.property(
                'unplannedAttr2',
            );
            transformedEvent.ProductImpressions[1].ProductList[1].Attributes.should.have.property(
                'allowedAttr1',
                'val4',
            );
            transformedEvent.ProductImpressions[1].ProductList[1].Attributes.should.have.property(
            ...
            transformedEvent.ProductImpressions[1].ProductList[1].Attributes.should.not.have.property(
                'unplannedAttr1',
            );
            transformedEvent.ProductImpressions[1].ProductList[1].Attributes.should.not.have.property(
                'unplannedAttr2',
    ...
    describe('kit blocking - integration tests', () => {
        ...
        it('integration test - should not throw an error when unplanned user attributes are allowed and block.ua = true', function (done) {
            ...
            expect(() => {
                kitBlocker.isAttributeKeyBlocked('unplannedAttr');
            }).to.not.throw(
                TypeError,
                /Cannot read properties of undefined \(reading 'unplannedAttr'\)/,
            );
            ...
            expect(kitBlocker.isAttributeKeyBlocked('unplannedAttr')).to.equal(
                false,
            );
            // reset data points
        ...
        it('integration test - should block an unplanned attribute set via setUserTag from being set on the forwarder if additionalProperties = false and blok.ua = true', function (done) {
            ...
            window.mParticle.init(apiKey, window.mParticle.config);
            waitForCondition(hasIdentifyReturned).then(() => {
                window.mParticle.Identity.getCurrentUser().setUserTag(
                    'unplannedAttr',
                    true,
                );
                window.MockForwarder1.instance.should.have.property(
        ...
        describe('integration tests - user identity related', () => {
            ...
            it('integration test - should not block any unplanned user identities to the forwarder when blok.id = true and additionalProperties = true', function (done) {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    waitForCondition(() => {
                        ...
                        event.UserIdentities.find(
                            (UI) => UI.Type === 4,
                        ).should.have.property('Identity', 'GoogleId');
            ...
            it('integration test - should block user identities to the forwarder when additional properties = false and blok.id = true', function (done) {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    waitForCondition(() => {
                        ...
                        (
                            event.UserIdentities.find((UI) => UI.Type === 4) ===
                            undefined
                        ).should.not.have.property('Identity', 'GoogleId');
            ...
            it('integration test - should not block identities from being passed to onUserIdentified/onLogoutComplete if blok.id = true and additionalProperties = true (in addition to having a filtered user identity list)', function (done) {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    waitForCondition(() => {
                        ...
                        const onUserIdentifiedUserIdentities =
                            window.MockForwarder1.instance.onUserIdentifiedUser.getUserIdentities()
                                .userIdentities;
                        onUserIdentifiedUserIdentities.should.not.have.property(
                            'google',
                        );
                        onUserIdentifiedUserIdentities.should.have.property(
                            'customerid',
                            'id1',
                        );
                        onUserIdentifiedUserIdentities.should.have.property(
                        ...
                        const onLogoutCompleteUserIdentities =
                            window.MockForwarder1.instance.onLogoutCompleteUser.getUserIdentities()
                                .userIdentities;
                        onLogoutCompleteUserIdentities.should.not.have.property(
                            'google',
                        );
                        onLogoutCompleteUserIdentities.should.have.property(
                            'customerid',
                            'id1',
                        );
                        onLogoutCompleteUserIdentities.should.have.property(
            ...
            it('integration test - should not block identities from being passed to onUserIdentified/onModifyComplete if blok.id = true and additionalProperties = true (in addition to having a filtered user identity list)', function (done) {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    waitForCondition(() => {
                        ...
                        const onModifyCompleteUserIdentities =
                            window.MockForwarder1.instance.onModifyCompleteUser.getUserIdentities()
                                .userIdentities;
                        onModifyCompleteUserIdentities.should.not.have.property(
                            'google',
                        );
                        onModifyCompleteUserIdentities.should.have.property(
                            'customerid',
                            'id1',
                        );
                        onModifyCompleteUserIdentities.should.have.property(
            ...
            it('integration test - should not block identities from being passed to onUserIdentified/onIdentifyComplete if blok.id = true and additionalProperties = true (in addition to having a filtered user identity list)', function (done) {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    const onIdentifyCompleteUserIdentities =
                        window.MockForwarder1.instance.onIdentifyCompleteUser.getUserIdentities()
                            .userIdentities;
                    onIdentifyCompleteUserIdentities.should.not.have.property(
                        'google',
                    );
                    onIdentifyCompleteUserIdentities.should.have.property(
                        'customerid',
                        'id1',
                    );
                    onIdentifyCompleteUserIdentities.should.have.property(
            ...
            it('integration test - should not block identities from being passed to onUserIdentified/onLoginComplete if blok.id = true and additionalProperties = true (in addition to having a filtered user identity list)', function (done) {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    waitForCondition(() => {
                        ...
                        const onLoginCompleteUserIdentities =
                            window.MockForwarder1.instance.onLoginCompleteUser.getUserIdentities()
                                .userIdentities;
                        onLoginCompleteUserIdentities.should.not.have.property(
                            'google',
                        );
                        onLoginCompleteUserIdentities.should.have.property(
                            'customerid',
                            'id1',
                        );
                        onLoginCompleteUserIdentities.should.have.property(
        ...
        describe('integration tests - product attribute related', () => {
            ...
            beforeEach(() => {
                ...
                product1 = window.mParticle.eCommerce.createProduct(
                    'iphone',
                    'iphoneSKU',
                    999,
                    1,
                ...
                product2 = window.mParticle.eCommerce.createProduct(
                    'galaxy',
                    'galaxySKU',
                    799,
                    1,
            ...
            it('integration test - should block any unplanned product attributes from reaching the forwarder if additionalProperties = false and block.ea=true', function (done) {
                waitForCondition(hasIdentifyReturned).then(() => {
                    ...
                    const event = window.MockForwarder1.instance.receivedEvent;
                    const products = event.ProductAction.ProductList;

                    products[0].Attributes.should.have.property(
                        'plannedAttr1',
                        'val1',
                    );
                    products[0].Attributes.should.have.property(
                    ...
                    products[0].Attributes.should.not.have.property(
                        'unplannedAttr1',
                    );

                    products[1].Attributes.should.have.property(
                        'plannedAttr1',
                        'val1',
                    );
                    products[1].Attributes.should.have.property(
                    ...
                    products[1].Attributes.should.not.have.property(
                        'unplannedAttr1',
                    );
        ...
        describe('integration tests - client passed in data plan', () => {
            const clientProvidedDataPlan: DataPlanVersion = {
                version_document: {
                    data_points: [
                        {
                            match: {
            ...
            it('integration test - should prioritize data plan from config.dataPlanOptions over server provided data plan', function (done) {
                ...
                waitForCondition(hasIdentifyReturned).then(() => {
                    logs.includes(
                        'Customer provided data plan found',
                    ).should.equal(true);
                    logs.includes(
                        'Data plan found from mParticle.js',
                    ).should.equal(false);
                    logs.includes('Data plan found from /config').should.equal(
                        false,
                    );
            ...
            it('should log an error if the data plan has an error on it', async () => {
                ...
                fetchMock.get(
                    `${urls.config}&plan_id=robs_plan&plan_version=1`,
                    {
                        status: 200,
                        body: JSON.stringify({
                            dataPlanResult: { error_message: errorMessage },
                        }),
                    },
                );
