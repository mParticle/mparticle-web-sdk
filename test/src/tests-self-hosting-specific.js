...
describe('/config self-hosting integration tests', function () {
    it('queues events in the eventQueue while /config is in flight, then processes them afterwards with correct MPID', function (done) {
        ...
        mockServer.respondWith('https://identity.mparticle.com/v1/identify', [200, {}, JSON.stringify({ mpid: 'identifyMPID', is_logged_in: false })]);
        mockServer.respondWith('https://jssdkcdns.mparticle.com/JS/v2/test_key/config?env=0', [200, {}, JSON.stringify({})]);
    ...
    it('queued events contain login mpid instead of identify mpid when calling login immediately after mParticle initializes', function (done) {
        ...
        mockServer.respondWith('https://jssdkcdns.mparticle.com/JS/v2/test_key/config?env=0', [200, {}, JSON.stringify({})]);
        mockServer.respondWith('https://identity.mparticle.com/v1/identify', [200, {}, JSON.stringify({ mpid: 'identifyMPID', is_logged_in: false })]);
        mockServer.respondWith('https://identity.mparticle.com/v1/login', [200, {}, JSON.stringify({ mpid: 'loginMPID', is_logged_in: false })]);
        ...
        messages.indexOf('Parsing "login" identity response from server').should.be.above(0);
        
        // when login is in flight, identify will not run, but callback still will
        messages.indexOf('Parsing "identify" identity response from server').should.equal(-1);
