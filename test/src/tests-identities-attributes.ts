...
describe('identities and attributes', function () {
    ...
    it('should set and log position', function (done) {
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            expect(event.data.location).to.have.property(
                'longitude',
                -118.321694,
            );
    ...
    it('should get user attribute lists', function (done) {
        ...
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            const userAttributes =
                mParticle.Identity.getCurrentUser().getUserAttributesLists();
    ...
    it('should get cart products', function (done) {
        ...
        const product2: SDKProduct = mParticle.eCommerce.createProduct(
            'Android',
            'SKU2',
            1,
            1,
    ...
    it('should send user identity change requests when setting new identities on new users', function (done) {
        ...
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            waitForCondition(() => {
                ...
                expect(typeof event.data.new.timestamp_unixtime_ms).to.equal(
                    'number',
                );
                ...
                expect(typeof event.data.old.timestamp_unixtime_ms).to.equal(
                    'number',
                );
                ...
                waitForCondition(() => {
                    ...
                    expect(event2.data.new.identity_type).to.equal(
                        'customer_id',
                    );
                    expect(event2.data.new.identity).to.equal('customerid2');
                    expect(
                        typeof event2.data.new.timestamp_unixtime_ms,
                    ).to.equal('number');
                    expect(event2.data.new.created_this_batch).to.equal(false);
                    expect(event2.data.old.identity_type).to.equal(
                        'customer_id',
                    );
                    expect(event2.data.old.identity).to.equal('customerid1');
                    expect(
                        typeof event2.data.old.timestamp_unixtime_ms,
                    ).to.equal('number');
                    ...
                    waitForCondition(() => {
                        ...
                        expect(event3.event_type).to.equal(
                            'user_identity_change',
                        );
                        expect(event3.data.new.identity_type).to.equal('email');
                        expect(event3.data.new.identity).to.equal(
                            'test@test.com',
                        );
                        expect(
                            typeof event3.data.new.timestamp_unixtime_ms,
                        ).to.equal('number');
                        expect(event3.data.new.created_this_batch).to.equal(
                            true,
                        );

                        expect(event3.data.old.identity_type).to.equal('email');
                        expect(event3.data.old.identity === null).to.equal(
                            true,
                        );
                        expect(
                            typeof event3.data.old.timestamp_unixtime_ms,
                        ).to.equal('number');
                        expect(event3.data.old.created_this_batch).to.equal(
                            false,
                        );
                        ...
                        waitForCondition(() => {
                            ...
                            expect(event4.event_type).to.equal(
                                'user_identity_change',
                            );
                            expect(event4.data.new.identity_type).to.equal(
                                'other',
                            );
                            expect(event4.data.new.identity).to.equal('other1');
                            expect(
                                typeof event4.data.new.timestamp_unixtime_ms,
                            ).to.equal('number');
                            expect(event4.data.new.created_this_batch).to.equal(
                                true,
                            );

                            expect(event4.data.old.identity_type).to.equal(
                                'other',
                            );
                            expect(event4.data.old.identity === null).to.equal(
                                true,
                            );
                            expect(
                                typeof event4.data.old.timestamp_unixtime_ms,
                            ).to.equal('number');
                            expect(event4.data.old.created_this_batch).to.equal(
                                false,
                            );

                            mParticle.logEvent('testAfterLogout');

                            const body5 = JSON.parse(
                                `${fetchMock.lastOptions().body}`,
                            );
                            expect(body5.mpid).to.equal('mpid2');
                            expect(
                                Object.keys(body5.user_identities).length,
                            ).to.equal(1);
                            expect(body5.user_identities).to.have.property(
                                'other',
                                'other1',
                            );
    ...
    it('should order user identity change events before logging any events', async () => {
        ...
        const secondCallBody = JSON.parse(
            secondCall[1].body as unknown as string,
        ) as Batch;
        expect(secondCallBody.events[0].event_type).to.equal(
            'session_start',
            'Second Call',
        );
        ...
        const thirdCallBody = JSON.parse(
            thirdCall[1].body as unknown as string,
        ) as Batch;
        expect(thirdCallBody.events[0].event_type).to.equal(
        ...
        const fourthCallBody = JSON.parse(
            fourthCall[1].body as unknown as string,
        ) as Batch;
        expect(fourthCallBody.events[0].event_type).to.equal(
            'user_identity_change',
            'Fourth Call',
        );
        ...
        const fifthCallBody = JSON.parse(
            fifthCall[1].body as unknown as string,
        ) as Batch;
        const fifthCallEvent = fifthCallBody.events[0] as CustomEvent;
        expect(fifthCallEvent.event_type).to.equal(
            'custom_event',
            'Fifth Call',
        );
        expect(fifthCallEvent.data.event_name).to.equal(
            'Test Event 1',
            'Fifth Call',
        );
        ...
        const sixthCallBody = JSON.parse(
            sixthCall[1].body as unknown as string,
        ) as Batch;
        const sixthCallEvent = sixthCallBody.events[0] as CustomEvent;
        expect(sixthCallBody.events[0].event_type).to.equal(
            'custom_event',
            'Sixth Call',
        );
        expect(sixthCallEvent.data.event_name).to.equal(
            'Test Event 2',
            'Sixth Call',
        );
        ...
        const seventhEventBody = JSON.parse(
            seventhEvent[1].body as unknown as string,
        ) as Batch;
        const seventhEventEvent = seventhEventBody.events[0] as CustomEvent;
        expect(seventhEventBody.events[0].event_type).to.equal(
            'custom_event',
            'Seventh Call',
        );
        expect(seventhEventEvent.data.event_name).to.equal(
            'Test Event 3',
            'Seventh Call',
        );
    });
        ...
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            waitForCondition(() => {
                ...
                waitForCondition(() => {
                    ...
                    expect(batch.user_identities).to.not.have.property(
                        'customer_id',
                    );
    ...
    it('should not send user attribute change requests when user attribute already set with same value with false values', function (done) {
        ...
        waitForCondition(hasIdentifyReturned).then(() => {
            ...
            expect(body3.user_attributes).to.have.property(
                'testEmptyString',
                '',
            );
            const event3 = body3.events[0];
