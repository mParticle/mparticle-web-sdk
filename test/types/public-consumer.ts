...
import mParticle, {
    ...
    type RoktAttributeValue,
    type RoktAttributeValueArray,
    type RoktAttributeValueType,
    type RoktAttributes,
    type RoktPartnerExtensionData,
    type RoktPlacement,
    type RoktPlacementEvent,
    type RoktSelectPlacementsOptions,
    type RoktSubscriber,
    type RoktUnsubscriber,
    type SDKEventAttrs,
...
// @ts-expect-error Only the DT-compatible MPSideloadedKit type is public.
import type { IMPSideloadedKit } from '@mparticle/web-sdk';

// @ts-expect-error Only the DT-compatible MPSideloadedKit type is public.
import type { IMPSideloadedKitConstructor } from '@mparticle/web-sdk';

// @ts-expect-error DataPlanResult was not part of the DefinitelyTyped public API.
import type { DataPlanResult } from '@mparticle/web-sdk';

// @ts-expect-error RoktEventChannel was not part of the DefinitelyTyped public API.
import type { RoktEventChannel } from '@mparticle/web-sdk';

// @ts-expect-error RoktSubscription was not part of the DefinitelyTyped public API.
import type { RoktSubscription } from '@mparticle/web-sdk';

const roktAttributeValue: RoktAttributeValue = ['sku-1', 3, true];
const roktAttributeValueType: RoktAttributeValueType = null;
const roktAttributeValueArray: RoktAttributeValueArray = ['sku-1', 3, true];
const roktAttributes: RoktAttributes = {
    email: 'name@example.com',
    products: roktAttributeValueArray,
    value: roktAttributeValue,
    nullable: roktAttributeValueType,
};
const roktOptions: RoktSelectPlacementsOptions = {
    attributes: roktAttributes,
    identifier: 'prod.rokt.conf',
};
const roktPlacement = {} as RoktPlacement;
const roktPlacementEvent: RoktPlacementEvent<{ accepted: boolean }> = {
    body: { accepted: true },
    event: 'OFFER_ENGAGEMENT',
    placement: roktPlacement,
};
const roktUnsubscriber: RoktUnsubscriber = {
    unsubscribe() {},
};
const roktSubscriber: RoktSubscriber<
    RoktPlacementEvent<{ accepted: boolean }>
> = {
    subscribe(callback) {
        callback(roktPlacementEvent);
        return roktUnsubscriber;
    },
};
const roktExtensionData: RoktPartnerExtensionData<Record<string, unknown>> = {
    extension: {
        key: 'value',
    },
};
