...
export default function Events(mpInstance) {
    ...
    this.logEvent = function (event, options) {
        ...
        if (mpInstance._Helpers.canLog()) {
            const uploadObject =
                mpInstance._ServerModel.createEventObject(event);
    ...
    this.startTracking = function (callback) {
        if (!mpInstance._Store.isTracking) {
            if ('geolocation' in navigator) {
                mpInstance._Store.watchPositionId =
    ...
    this.logCheckoutEvent = function (step, option, attrs, customFlags) {
        const event =
            mpInstance._Ecommerce.createCommerceEventObject(customFlags);
    ...
    this.logProductActionEvent = function (
        ...
        if (event) {
            event.EventCategory =
                mpInstance._Ecommerce.convertProductActionToEventType(
                    productActionType,
                );
            event.EventName +=
                mpInstance._Ecommerce.getProductActionEventName(
                    productActionType,
                );
            event.ProductAction = {
    ...
    this.logPurchaseEvent = function (
        ...
        if (event) {
            ...
            event.ProductAction.ProductList =
                mpInstance._Ecommerce.buildProductList(event, product);
    ...
    this.logPromotionEvent = function (
        ...
        if (event) {
            event.EventName +=
                mpInstance._Ecommerce.getPromotionActionEventName(
                    promotionType,
                );
            event.EventCategory =
                mpInstance._Ecommerce.convertPromotionActionToEventType(
                    promotionType,
                );
