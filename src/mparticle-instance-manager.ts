...
if (!Array.isArray) {
    // @ts-expect-error Array.isArray polyfill assignment is not on the Array constructor type
...
function mParticleInstanceManager(this: IMParticleInstanceManager) {
    ...
    this.init = function (apiKey, config, instanceName) {
        ...
        instanceName = (
            !instanceName || instanceName.length === 0 ? Constants.DefaultInstance : instanceName
        ).toLowerCase();
    ...
    this.eCommerce = {
        ...
        createTransactionAttributes: function (id, affiliation, couponCode, revenue, shipping, tax) {
            return self
                .getInstance()
                .eCommerce.createTransactionAttributes(id, affiliation, couponCode, revenue, shipping, tax);
        },
    ...
    this.Consent = {
        ...
        createGDPRConsent: function (consented, timestamp, consentDocument, location, hardwareId) {
            return self
                .getInstance()
                .Consent.createGDPRConsent(consented, timestamp, consentDocument, location, hardwareId);
        },
        createCCPAConsent: function (consented, timestamp, consentDocument, location, hardwareId) {
            return self
                .getInstance()
                .Consent.createGDPRConsent(consented, timestamp, consentDocument, location, hardwareId);
        },
