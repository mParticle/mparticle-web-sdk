...
import { FetchUploader, XHRUploader, AsyncUploader, IFetchPayload } from './uploaders';
...
export interface IAudienceMembershipsServerResponse {
    ...
    ct: number; // timestamp
    id: string;
    audience_memberships: Array<Audience>;
}

export interface IAudienceMemberships {
    currentAudienceMemberships: Array<Audience>;
}
...
export default class AudienceManager {
    ...
    public async sendGetUserAudienceRequest(mpid: string, callback: (userAudiences: IAudienceMemberships) => void) {
        ...
        try {
            const userAudiencePromise: Response = await this.userAudienceAPI.upload(fetchPayload, audienceURLWithMPID);

            if (userAudiencePromise.status >= 200 && userAudiencePromise.status < 300) {
                ...
                throw new Error(`Uncaught HTTP Error ${userAudiencePromise.status}.`);
            }
        } catch (e) {
