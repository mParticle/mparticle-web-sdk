...
Example:
    ...
    Before: /v1/us1-abc123def456.../identify  →  After: /v1/us1-[a-f0-9]+/identify
"""
...
def sanitize_url(url: str) -> str:
    ...
    - /v1/us1-abc123.../identify -> /v1/us1-[a-f0-9]+/identify
    - /v3/JS/us1-xyz789.../events -> /v3/JS/us1-[a-f0-9]+/events
...
def rename_body_file(old_filename: str, new_filename: str, base_path: Path) -> bool:
    ...
    if not old_path.exists():
        print(f"[WARNING] Body file not found: {old_path}")
        return False
    
    if new_path.exists():
        print(f"[WARNING] Target body file already exists: {new_path}")
        return False
    
    try:
        old_path.rename(new_path)
        print(f"[RENAME] Renamed body file: {old_filename} -> {new_filename}")
        return True
    except Exception as e:
        print(f"[ERROR] Error renaming body file: {e}")
        return False
...
def rename_mapping_file(mapping_path: Path, test_name: str = None) -> Tuple[Path, bool]:
    ...
    if test_name:
        ...
        if new_path.exists() and new_path != mapping_path:
            print(f"[WARNING] Target mapping file already exists: {new_path}")
            return (mapping_path, False)
        
        if new_filename != filename:
            try:
                mapping_path.rename(new_path)
                print(f"[RENAME] Renamed mapping file: {filename} -> {new_filename}")
                return (new_path, True)
            except Exception as e:
                print(f"[ERROR] Error renaming mapping file: {e}")
                return (mapping_path, False)
...
def sanitize_mapping_data(mapping_data: Dict[str, Any], mapping_path: Path, test_name: str = None) -> Tuple[Dict[str, Any], bool]:
    ...
    if original_url:
        ...
        if original_url != sanitized_url:
            print(f"[SANITIZE] Sanitized API key from URL:")
    ...
    if original_body_filename:
        ...
        if old_body_filename != new_body_filename:
            ...
            if rename_body_file(old_body_filename, new_body_filename, mapping_path):
                ...
                print(f"[SANITIZE] Updated body filename reference in mapping:")
...
def save_mapping_file(mapping_data: Dict[str, Any], mapping_path: Path) -> None:
    ...
    try:
        ...
        print(f"[SUCCESS] Saved updated mapping file: {mapping_path}")
    except Exception as e:
        print(f"[ERROR] Error saving mapping file: {e}")
        sys.exit(1)
...
Examples:
  ...
  python3 sanitize_mapping.py wiremock-recordings/mappings/mapping-v1-us1-abc123-identify.json --test-name identify
  python3 sanitize_mapping.py wiremock-recordings/mappings/mapping-v3-us1-abc123-events.json --test-name log-event
        """
    )
    ...
    parser.add_argument(
        ...
        dest='test_name',
        required=True,
        help='Test name to use for renaming mapping and body files (e.g., "log-event", "identify")'
    )
    
    args = parser.parse_args()
    
    print(f"[INFO] Processing mapping file: {args.mapping_file}")
    print(f"[INFO] Test name: {args.test_name}")
    print()
    ...
    if data_changes_made or file_was_renamed:
        print("[SUCCESS] Sanitization complete!")
        if file_was_renamed:
            print("   - Mapping file renamed")
        if data_changes_made:
            print("   - Mapping data updated")
    else:
        print("[INFO] No changes needed - mapping is already sanitized")
