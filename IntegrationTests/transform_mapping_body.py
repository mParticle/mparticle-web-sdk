...
# These are Web SDK specific dynamic fields that change between runs
DEFAULT_REPLACE_FIELDS = [
    # Session and timing fields
    'session_id',           # Session ID
    'session_start_unixtime_ms',  # Session start timestamp
    'timestamp_unixtime_ms',      # Event timestamp
    'ct',                   # Creation time
    ...
    'device_application_stamp',   # Device application stamp (long form)
    'device_utc_offset',    # Device timezone offset
    
    # Identity fields
    'id',                   # Various IDs (event, message)
    'mpid',                 # mParticle ID (may vary in test)
    'previous_mpid',        # Previous MPID
    'request_id',           # Request ID
    'request_timestamp_ms', # Request timestamp
    
    # SDK fields
    'sdk_version',          # SDK version string
    'client_generated_id',  # Client generated UUID
    
    # Batch fields
    'source_request_id',    # Batch source request ID
    
    # Web SDK specific fields
    'cgid',                 # Client Generated ID
    'dbg',                  # Debug mode (can vary)
    
    # Legacy/iOS compatible fields (in case of shared mappings)
    'a',                    # App ID
...
def load_mapping_file(mapping_file: str) -> tuple[Path, Dict[str, Any]]:
    ...
    try:
        ...
        print(f"[ERROR] Error reading mapping file: {e}")
        sys.exit(1)
...
def get_request_body_from_mapping(mapping_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[ERROR] Error extracting body from mapping: {e}")
        sys.exit(1)
...
def set_request_body_in_mapping(mapping_data: Dict[str, Any], request_body: Dict[str, Any], as_string: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[ERROR] Error setting body in mapping: {e}")
        sys.exit(1)
...
def mode_unescape(mapping_data: Dict[str, Any], mapping_path: Path) -> None:
    ...
    print(f"[INFO] Mapping file: {mapping_path}")
    print(f"[TRANSFORM] Converting equalToJson from string to formatted JSON object...")
    ...
    try:
        ...
        print(f"[SUCCESS] Mapping file updated successfully!")
        print(f"[INFO] equalToJson is now a formatted JSON object:\n")
        print(json.dumps(request_body, indent=2, ensure_ascii=False))
        
    except Exception as e:
        print(f"[ERROR] Error saving updated mapping file: {e}")
        sys.exit(1)
...
def mode_escape(mapping_data: Dict[str, Any], mapping_path: Path) -> None:
    ...
    try:
        ...
        if isinstance(equal_to_json, str):
            print(f"[INFO] Mapping file: {mapping_path}")
            print(f"[INFO] equalToJson is already a string (escaped format)")
            print(f"[SUCCESS] No action needed")
            return
        
        # It's a JSON object, convert it to string
        print(f"[INFO] Mapping file: {mapping_path}")
        print(f"[TRANSFORM] Converting equalToJson from JSON object to escaped string...")
        ...
        print(f"[SUCCESS] Mapping file updated successfully!")
        print(f"[INFO] equalToJson is now an escaped string")
...
def mode_unescape_update(mapping_data: Dict[str, Any], mapping_path: Path) -> None:
    ...
    print(f"[INFO] Mapping file: {mapping_path}")
    print(f"[TRANSFORM] Replacing {len(DEFAULT_REPLACE_FIELDS)} dynamic fields with ${{json-unit.ignore}}...")
    ...
    try:
        ...
        print(f"[SUCCESS] Mapping file updated successfully!")
        print(f"[INFO] Updated body with replaced fields:\n")
